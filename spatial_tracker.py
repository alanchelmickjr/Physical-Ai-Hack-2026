#!/usr/bin/env python3
"""Spatial Tracker - OAK-D depth-enabled person tracking (VPU YOLO)

Uses OAK-D Pro's Myriad X VPU for YOLO inference + stereo depth.
Provides 3D positions for detected people.
...
# YOLO model blob URL (YOLOv8n for OAK-D VPU)
...
class SpatialTracker:
    ...
    def _create_pipeline(self) -> dai.Pipeline:
        ...
        cam_rgb.setPreviewSize(640, 352)  # Match YOLO input size
        ...
        spatial_nn.setDepthLowerThreshold(100)   # 10cm min
        spatial_nn.setDepthUpperThreshold(10000) # 10m max

        # YOLOv8 specific settings
        spatial_nn.setNumClasses(80)
        spatial_nn.setCoordinateSize(4)
        spatial_nn.setAnchors([])  # YOLOv8 is anchor-free
    ...
    def start(self) -> bool:
        """Start the spatial tracker with YOLO on VPU."""
        try:
            ...
            print("[SpatialTracker] Started with YOLO on VPU + stereo depth")
            return True
    ...
    def _process_loop(self):
        """Main processing loop - gets detections from VPU."""
        while self._running:
            try:
                # Get frames
                ...
                if det_msg is not None:
                    self._process_vpu_detections(det_msg)
    ...
    def _process_vpu_detections(self, det_msg):
        """Process detections from VPU YOLO."""
        detections = []
